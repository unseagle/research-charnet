# This is a modified version of test_net.py in the original CharNet repository.
# The code below generates character bounding box labels as expected by place_text.py
...
def save_char_recognition(char_bboxes, image_id, save_root, separator=chr(31)):
    with open('{}/{}.txt'.format(save_root, image_id), 'a') as fw:
        for bb in char_bboxes:
            fw.write(separator.join([str(int(_)) for _ in bb[:-1]]))
            fw.write('\n')


def decode(char_scores):
    char_dict = load_char_dict("datasets/ICDAR2015/test/char_dict.txt")
...
def save_img(bboxes, img, img_name):
    for bb in bboxes:
        pts = np.array(list(zip(bb[::2], bb[1::2])), dtype=np.int32)
        cv.polylines(img, [pts], isClosed=True, color=(255, 0, 0), thickness=1)
...
if __name__ == '__main__':
    ...
    for im_name in sorted(os.listdir(args.image_dir)):
        if im_name[-4:] != '.png':
            # Some of the prior generated images are corrupted (no file extension).
            continue
